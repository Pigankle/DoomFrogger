...
class HistoryPlots():
    ...
    def __init__(self):
        self.colorlookup = {x.objecttype: self.hextriplet(
            x.color) for x in CAR_TYPES}
    ...
    def make_threat_piechart(self, df):
        ...
        ax1.pie(gb_threat["PosY"],
                shadow=False, startangle=90, colors=color_list)
    ...
    def make_blinderct_timeline(self, df):
        ...
        fig, ax = plt.subplots(
            figsize=(SCREEN_WIDTH*px*0.8, SCREEN_HEIGHT * px * 0.1))
        ax.plot(df['Time'], df['PrevBlinderCount'], color='black')
        ax.scatter(df['Time'], df['PrevBlinderCount'],
                   c=df['HitType'].map(self.colorlookup), zorder=3)
        return plt
